...
{
    public sealed class GraphQLStringTypeAdapter : IGraphQLTypeAdapter
    {
        public CoercionResult CoerceInput(Type targetType, string name, object value)
        {
            return value is string stringValue
                ? new CoercionResult(stringValue, null)
                : value == null
                ? new CoercionResult(value, null)
                : new CoercionResult(
                    null,
        ...
        {
            return new CoercionResult(value?.ToString());
        }

        public GraphQLTypeDescription GetTypeDescription(Type targetType)
        {
            return GraphQLTypeDescription.Named("String");
        }
    }
}
